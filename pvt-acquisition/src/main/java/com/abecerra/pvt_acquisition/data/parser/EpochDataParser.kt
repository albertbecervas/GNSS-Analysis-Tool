package com.abecerra.pvt_acquisition.data.parser
...
import com.abecerra.pvt_computation.domain.computation.utils.Constants.L1_FREQ
import com.abecerra.pvt_computation.suplclient.ephemeris.EphemerisResponse
import com.abecerra.pvt_computation.utils.PvtConstants.GAL_E1C
import com.abecerra.pvt_computation.utils.PvtConstants.UNCERTAINTY_THR
import com.abecerra.pvt_computation.utils.PvtConstants.WEEK_NANOS
...
object EpochDataParser {
    ...
    private fun GnssMeasurement.getSatellite(timeNanosGnss: Double): SatelliteMeasurements? {
        return if (!hasMultiPathOrUncertainty(
                this
            )
        ) {
            buildSatellite(
    ...
    private fun getRxTime(state: Int, timeNanosGnss: Double): Double? {
        return when {
            checkTowDecode(state) || checkTowKnown(
                state
            ) -> {
